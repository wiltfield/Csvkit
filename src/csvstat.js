...
const persist = debounce(() => {
  ...
  saveDraft(DRAFT_KEY, { parsedRows, currentOutputRows, lastStatsSerialized });
});
...
worker.onmessage = (e) => {
  ...
  if (awaitingParse) {
    ...
    renderTable(rows);
    term.say('File is ready. Get stats when you\u2019re set.');
    save.hide();
    persist();
    return;
  }
  ...
  renderTable(rows);
  if (lastStatsSerialized !== null && serialized === lastStatsSerialized) {
    term.error('Stats haven\u2019t changed.');
  } else {
    lastStatsSerialized = serialized;
    currentOutputRows = rows;
    save.show();
    term.say('Stats computed.');
    persist();
  }
};
...
runBtn.addEventListener('click', () => {
  ...
  term.say('Computing stats...');
  worker.postMessage({ op: 'stat', rows: parsedRows });
});
...
(function restore() {
  ...
  if (draft && draft.parsedRows) {
    ...
    if (draft.currentOutputRows) {
      ...
      lastStatsSerialized = draft.lastStatsSerialized || null;
      save.show();
    }
